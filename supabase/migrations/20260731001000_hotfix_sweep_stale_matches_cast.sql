-- ============================================================
-- HOTFIX (ROLL-FORWARD) — CASTEO DE ENUM EN `sweep_stale_matches` — 2026-07-31
-- ------------------------------------------------------------
-- Detectado por `supabase test db` durante el despliegue del Bloque 9, en la
-- primera corrida real de `300-sweep-stale-matches.spec.sql`.
--
-- ── El bug ──────────────────────────────────────────────────────────────────
-- La rama de WO automático hacía:
--
--     update matches
--       set status = case when v_winner_team_id = r.team_a_id then 'WO_A' else 'WO_B' end
--
-- Un literal suelto (`set status = 'WO_A'`) se castea implícitamente al tipo de
-- la columna, pero **dentro de un CASE las dos ramas se resuelven a `text`** y
-- Postgres deja de coercionar. Resultado:
--
--     ERROR 42804: column "status" is of type match_status but expression is of type text
--
-- ── Por qué es más grave de lo que parece ───────────────────────────────────
-- La excepción aborta la función ENTERA, no sólo esa rama. Es decir: el primer
-- partido que califique para auto-WO no sólo no se resuelve — además impide que
-- esa corrida del barrido cancele los `PENDIENTE` sin coordinar (rama 1) y
-- cierre los `EN_VIVO` abandonados (rama 3). El cron horario queda inútil hasta
-- que alguien resuelva ese partido a mano.
--
-- ── Origen y por qué esta migración existe ──────────────────────────────────
-- La línea nació en `20260728181000` (D3/D4, Bloque 3) y se propagó literal a
-- `20260729121000` (E8, Bloque 8), que reescribió la función para sumarle la
-- rama de desafíos vencidos. Nunca se detectó porque la suite 300 abortaba en
-- su primer INSERT desde que se escribió (fixture sin `format`): dos bugs
-- tapándose mutuamente.
--
-- El fix se aplicó primero **in situ** sobre el archivo de E8, asumiendo que
-- esa migración todavía no había salido. `supabase db push --dry-run` mostró lo
-- contrario: `20260729121000` YA figura en el historial del remoto, así que
-- `db push` la saltea y la corrección in situ jamás llegaría a producción. De
-- ahí este roll-forward: es la única vía de que el fix impacte la base real.
--
-- Se verificó por introspección de `pg_proc` que producción corría la versión
-- sin cast. El cron `sweep-stale-matches` llevaba 20 corridas sin fallar: el
-- bug estaba **latente**, esperando el primer equipo que no se presentara.
--
-- ── Alcance ─────────────────────────────────────────────────────────────────
-- Cuerpo IDÉNTICO al de `20260729121000`, con el único cambio del cast
-- explícito. No toca umbrales, ni ramas, ni el jsonb de retorno. El job
-- `sweep-stale-matches` no se reprograma: sigue el que ya existe (`20 * * * *`).
-- ============================================================
...
COMMENT ON FUNCTION public.sweep_stale_matches() IS
  'Barrido horario de partidos huérfanos (D3/D4/E6) y de desafíos sin responder (E8). Idempotente: cada rama mueve la fila a un estado que ya no matchea su WHERE. Umbrales en app_settings (sweep_*). No toca partidos con un wo_claim en PENDIENTE_REVISION. El estado del auto-WO va casteado explícitamente a match_status (hotfix 20260731001000).';

-- Función de mantenimiento: jamás ejecutable desde la API REST.
