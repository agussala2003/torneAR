-- (ajuste 2026-07-14: se removió CONCURRENTLY — no puede correr en la
--  transacción de supabase db reset; en producción ya se aplicó con lock cero)
-- ============================================================
-- FK INDEXES — 2026-03-31
-- Adds missing indexes on foreign key columns for query performance.
-- NOTE: CREATE INDEX cannot run inside a transaction block.
...
CREATE INDEX IF NOT EXISTS idx_cancellation_requests_requested_by_team_id
...
CREATE INDEX IF NOT EXISTS idx_market_player_posts_profile_id
  ON market_player_posts(profile_id);

CREATE INDEX IF NOT EXISTS idx_market_team_posts_created_by
  ON market_team_posts(created_by);

CREATE INDEX IF NOT EXISTS idx_market_team_posts_team_id
  ON market_team_posts(team_id);

CREATE INDEX IF NOT EXISTS idx_match_dispute_votes_voted_team_id
  ON match_dispute_votes(voted_team_id);

CREATE INDEX IF NOT EXISTS idx_match_proposals_from_team_id
  ON match_proposals(from_team_id);

CREATE INDEX IF NOT EXISTS idx_match_proposals_match_id
  ON match_proposals(match_id);

CREATE INDEX IF NOT EXISTS idx_match_proposals_proposed_by
  ON match_proposals(proposed_by);

CREATE INDEX IF NOT EXISTS idx_match_proposals_venue_id
  ON match_proposals(venue_id);

CREATE INDEX IF NOT EXISTS idx_match_results_mvp_id
  ON match_results(mvp_id);

CREATE INDEX IF NOT EXISTS idx_match_results_submitted_by
...
CREATE INDEX IF NOT EXISTS idx_result_dispute_votes_voted_for_team
  ON result_dispute_votes(voted_for_team);

CREATE INDEX IF NOT EXISTS idx_wo_claims_claimed_by
  ON wo_claims(claimed_by);

CREATE INDEX IF NOT EXISTS idx_wo_claims_claiming_team_id
